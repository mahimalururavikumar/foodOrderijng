...
const CanteenDashboard = () => {
  ...
  const markOutOfStock = async (id) => {
    try {
      await axios.put(
        `http://localhost:5000/api/canteen/out-of-stock/${id}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      setItems(
        items.map((item) =>
          item._id === id ? { ...item, outOfStock: true } : item
        )
      );
    } catch (error) {
  ...
  const addToStock = async (id) => {
    try {
      await axios.put(
        `http://localhost:5000/api/canteen/add-to-stock/${id}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      setItems(
        items.map((item) =>
          item._id === id ? { ...item, outOfStock: false } : item
        )
      );
    } catch (error) {
  ...
  const handleAddItem = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/canteen/upload",
        formData,
        {
          headers: {
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className={`py-2 px-4 rounded-lg transition-all ${
              location.pathname === "/canteen-dashboard"
      ...
      <div className="mt-20 w-full max-w-6xl p-6 flex flex-wrap justify-center gap-6">
        ...
        {loading ? (
          ...
          items.map((item) => (
            <div
              key={item._id}
              className="bg-white/70 backdrop-blur-md p-4 rounded-xl shadow-lg w-64 transform transition duration-300 hover:scale-105"
            >
                           {" "}
              <img
                src={item.imageUrl}
              ...
              <span
                className={`inline-block px-3 py-1 mt-2 rounded text-white ${
